      ...
      <div class="glass rounded-2xl p-6 fade-in" style="animation-delay: 0.1s">
        ...
        <div class="bg-white/5 rounded-xl p-4 mb-4">
          <div class="flex items-center justify-between mb-1.5">
            <p class="text-xs text-surface-500">${isDiary ? 'Nội dung tiếng Việt:' : 'Nghĩa tiếng Việt:'}</p>
            <button id="toggle-vi"
              class="flex items-center gap-1 text-xs text-surface-500 hover:text-primary-400 transition-colors select-none">
              <svg id="toggle-vi-icon" class="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
              </svg>
              <span id="toggle-vi-label">Hiện gợi ý</span>
            </button>
          </div>
          <p id="vi-content"
             class="hidden text-surface-100 font-medium ${isDiary ? 'text-sm' : 'text-base'} leading-relaxed">
            ${currentExample.vi}
          </p>
        </div>
...
export function initWriteSentenceEvents(allWords, rerenderFn) {
  ...
  document.getElementById('toggle-vi')?.addEventListener('click', () => {
    const content = document.getElementById('vi-content');
    const label = document.getElementById('toggle-vi-label');
    const icon = document.getElementById('toggle-vi-icon');
    if (!content) return;
    const isHidden = content.classList.toggle('hidden');
    if (isHidden) {
      label.textContent = 'Hiện gợi ý';
      icon.innerHTML = `
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>`;
    } else {
      label.textContent = 'Ẩn gợi ý';
      icon.innerHTML = `
